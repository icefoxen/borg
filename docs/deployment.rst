...
The client will be able to access any file or subdirectory inside of ``/home/backup/repos/<client
fqdn>`` but no other directories.  You can allow a client to access several directories by passing
multiple `--restrict-to-path` flags, for instance: ``borg serve --restrict-to-path
/home/backup/repos/<client fqdn>/root --restrict-to-path /home/backup/repos/<client fqdn>/home``,
or instead simply use `--restrict-to-path` once to restrict the client to
``/home/backup/repos/<client fqdn>/*``.
